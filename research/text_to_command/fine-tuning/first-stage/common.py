...
def is_command_classification_correct_2(llm_answer, correct_answer) -> tuple:
    try:
        if type(correct_answer) is not list or len(correct_answer) < 1:
            raise ValueError("Correct answer must be a list with at least one element.")
        elif correct_answer[0] in ["GG_SCALE", "SPEED_REQUEST"] and len(correct_answer) != 3:
            raise ValueError("Correct answer for GG_SCALE and SPEED_REQUEST must be a list with three elements: [command_class, value, info].")
        elif type(llm_answer) is not str:
            raise ValueError("LLM answer must be a string.")
        else:
            llm_answer = llm_answer.split("\n")
            if len(llm_answer) < 1:
                raise ValueError("LLM answer must be at least a single line string.")
            elif correct_answer[0] in ["GG_SCALE", "SPEED_REQUEST"] and len(llm_answer) != 3:
                raise ValueError("LLM answer for GG_SCALE and SPEED_REQUEST must be a three line string.")
            else:
                predicted_command_class = llm_answer[0]
                correct_command_class = correct_answer[0]
                predicted_info = llm_answer[2] if len(llm_answer) == 3 else None
                correct_info = correct_answer[2] if len(correct_answer) == 3 else None
                predicted_value = correct_value = None

                try:
                    if correct_command_class == "SPEED_REQUEST":
                        predicted_value = int(llm_answer[1]) if len(llm_answer) == 3 else None
                        correct_value = int(correct_answer[1]) if len(correct_answer) == 3 else None
                    elif correct_command_class == "GG_SCALE":
                        predicted_value = float(llm_answer[1]) if len(llm_answer) == 3 else None
                        correct_value = float(correct_answer[1]) if len(correct_answer) == 3 else None
                except ValueError:
                    return ("INCORRECT_VALUE", predicted_command_class, predicted_value, predicted_info)

                if predicted_command_class == correct_command_class:
                    if predicted_value is None:
                        return ("CLASSIFICATION_CORRECT", predicted_command_class, predicted_value, predicted_info)
                    elif predicted_value == correct_value and predicted_info == correct_info:
                        return ("CLASSIFICATION_CORRECT", predicted_command_class, predicted_value, predicted_info)
                    elif predicted_value == correct_value:
                        return ("INCORRECT_INFO", predicted_command_class, predicted_value, predicted_info)
                    else:
                        return ("INCORRECT_VALUE", predicted_command_class, predicted_value, predicted_info)
                else:
                    return ("CLASSIFICATION_INCORRECT", predicted_command_class, predicted_value, predicted_info)
    except ValueError:
        return ("WRONG_OUTPUT_FORMAT")
...
def get_test_dataset(source_file_path: str) -> tuple:
    ...
    with open(os.path.join(os.path.dirname(__file__), 'data', 'prompt_template_1.txt')) as file:
...
def get_prompt_template(command_list=list(), prompt_version = 1) -> str:
    if prompt_version == 1:
       with open(os.path.join(os.path.dirname(__file__), 'data', 'prompt_template_1.txt'), 'r') as file:
        ...
        with open(os.path.join(os.path.dirname(__file__), 'data', 'prompt_template_2.txt'), 'r') as file:
            lines = file.readlines()
            header = lines[0:6]
        ...
        with open(os.path.join(os.path.dirname(__file__), 'data', 'prompt_template_3.txt'), 'r') as file:
            lines = file.readlines()
            header = lines[0:4]
